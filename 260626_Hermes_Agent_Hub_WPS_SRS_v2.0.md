...
- owner_email 후보
- source/platform
- Admin 발급 Agent API token

### 7.2 1안: Admin 발급 Agent API Token 기반 등록

Admin이 Hub에서 사용자 email 기준으로 Agent API token을 직접 발급한다.
이때 Hub의 `agent_uid`는 사용자 email과 동일한 값으로 고정한다.
Hermes Agent 또는 Collector는 최초 연결 시 별도 enrollment API를 호출하지 않고,
발급받은 token을 Agent 설정에 저장한 뒤 Bearer header로 사용한다.

```env
HERMES_HUB_URL=http://hub.local:8000
HERMES_AGENT_UID=kim@company.com
HERMES_API_TOKEN=hub_api_xxxxx
```

Hub는 token hash만 저장하며 token 원문은 발급 응답에서만 노출한다.

### 7.3 2안: Enrollment 또는 UNMAPPED 등록 후 Admin 승인
...
1. Admin 발급 Agent API Token 기반 등록
2. Enrollment Token 기반 자동 등록
3. Admin 수동 매핑

### 7.5 Agent API Token 사용 원칙

Admin이 Agent token을 발급하면 Hub는 Agent 전용 `api_token`을 1회 반환한다. Hub DB에는 token 원문을 저장하지 않고 `token_hash`만 저장한다.

Agent 또는 Collector는 모든 heartbeat, message ingest, event ingest 요청에 다음 HTTP header를 포함해야 한다.
...
| message_type_code | INTEGER | observer message type numeric code |
| content | TEXT | 메시지 본문 |
| assistant_response | TEXT NULL | LLM 응답 본문. `post_llm_call`에서 사용 |
...
Observer message type은 DB 저장 용량을 줄이기 위해 integer code로 저장한다.

| code | message_type | 설명 |
|---:|---|---|
| 1 | pre_llm_call | LLM 호출 직전 사용자 prompt |
| 2 | post_llm_call | LLM 응답 수신 후 |
| 3 | pre_tool_call | Tool 호출 직전 |
| 4 | post_tool_call | Tool 결과 수신 후 |

기존 `agent:start`, `agent:end` event type은 각각 `pre_llm_call`, `post_llm_call`로
추론하여 하위 호환한다.
...
| agent_messages | INDEX(message_type_code, occurred_at) | observer message type 검색 |
...
| Admin Agent Token API | Cookie Session | Admin이 email 기반 Agent UID와 Bearer token을 직접 발급 |
| Enrollment API | Enrollment Token | 대안 등록 흐름. Bearer token은 아직 없음 |
...
Admin이 특정 owner email 기준으로 active Agent API token을 직접 발급한다.
이때 `agent_uid`는 `owner_email`과 동일한 값으로 생성 또는 재사용한다.
...
Agent는 이후 `Authorization: Bearer hub_api_xxxxx` header로 heartbeat,
message ingest, event ingest를 호출한다.
...
실제 Hermes observer payload의 `conversation_history`는 turn마다 누적되는 list이므로
Hub raw payload 저장 대상에서 제외한다. Hub는 현재 turn의 `user_message` 또는
`assistant_response`를 normalized field로 저장하고, 필요 시 Hub DB의 `session_key`,
`request_id`, `occurred_at` 기준으로 대화 흐름을 재구성한다.
...
v2.0 초기 Admin Web Shell은 `/admin/login`, `/admin/dashboard`, `/admin/agents`,
`/admin/agent-tokens`, `/admin/messages`를 제공한다. Dashboard 첫 화면은 기존 Admin
session cookie를 사용해 `/admin/api/me`와 `/admin/api/dashboard/summary`를 조회하며,
인증되지 않은 사용자는 login 화면으로 이동한다. Agent Token 화면은
`/admin/api/agent-tokens`를 통해 email 기반 Agent UID와 Agent API token을 직접 발급한다.
...
message_type, event_type, keyword 필터를 적용하고, 목록에서 발생 시각, Agent,
owner, source, role, message type, event type, content preview를 확인할 수 있어야 한다.
Message row 선택 시
Message 상세 drawer가 열리고 `/admin/api/messages/{message_id}`를 사용해 본문,
assistant_response, session, request/response 연결 정보, tool call JSON, raw payload
JSON을 확인할 수 있어야 한다. 상세 drawer의 related message 항목은 같은 drawer
안에서 재조회할 수 있어야 한다.
...
### FR-003 Agent API Token 발급

Admin은 특정 email 기준으로 Agent API token을 직접 발급할 수 있어야 한다.
발급 시 Hub는 `agent_uid = email`인 Agent를 생성 또는 재사용해야 한다.
...
Hermes observer message type은 `pre_llm_call`, `post_llm_call`, `pre_tool_call`,
`post_tool_call` 4가지를 지원하고 DB에는 `message_type_code` integer로 저장해야 한다.
`post_llm_call` payload는 LLM 응답 본문을 `assistant_response`에도 저장해야 한다.

### FR-009 Raw Payload 저장

모든 ingest message/event는 원본 payload JSON을 저장해야 한다.
단, Hermes observer의 `conversation_history`는 turn마다 누적되어 중복과 민감정보
노출면을 키우므로 raw payload 저장에서 제외해야 한다.

### FR-010 Normalized Field 저장

검색 및 필터링을 위해 source, role, event_type, message_type_code, content,
assistant_response, occurred_at 등 normalized field를 저장해야 한다.
...
| 항목 | 설명 |
|---|---|
| agent_uid | Hub 기준 Agent ID. Admin direct token 발급 흐름에서는 사용자 email과 동일 |
...
- Login
- Agent API token 인증
- Admin direct Agent API token 발급
- Agent enrollment 대안 흐름
- Message ingest
- Event ingest
- Idempotency 처리
- Message Explorer
...
- Agent UID(email) 및 Agent API token 설정
- e2e message 저장 확인
...
### AC-002 Agent Token 발급 및 등록

Admin은 사용자 email 기준으로 Agent API token을 발급할 수 있어야 한다.
이때 Hub는 `agent_uid = email`인 ACTIVE Agent를 생성 또는 재사용해야 한다.

Hermes Agent 또는 Collector는 발급받은 Agent UID와 Bearer API token으로 Hub API를 호출할 수 있어야 한다.
...
Admin은 Agent API token 발급, Agent 목록 조회, UNMAPPED Agent 매핑, 표시 이름 변경, Agent 비활성화를 수행할 수 있어야 한다.
